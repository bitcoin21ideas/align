...
## Phase 0 — Hardening before Discord
*Goal: reduce public-repo and operational risk before adding new surfaces.*

- [ ] Secure the n8n webhook `S`
  Add Webhook node auth, rotate the path, update the iOS Shortcut, and run one full recording.

- [x] Sanitize public workflow exports `S`
  Raw n8n exports stay in `workflows/.local/`; the committed workflow uses placeholders and passes `scripts/check-public-workflow.mjs`.

- [ ] Prove no-memory append skip `S`
  Confirm a no-memory run writes no monthly memory heading and no memory-index commit.

- [ ] Add deeper workflow invariant checks `M`
  Enforce webhook auth, credential placeholders, storage repo placeholders, prompt loading, and memory-branch shape in CI.
...
- [ ] `#walk-briefs` post-run summary `S`
  ...
  No new workflow — appended to the existing run. Starts after Phase 0 webhook
  auth and no-memory append testing.
